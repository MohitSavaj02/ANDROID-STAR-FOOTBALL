        ...
        if (response.isSuccessful) {
            ...
            val body: ErrorBean = if (errorBody != null) {
                ...
                if (jsonObject.has("detail"))
                    errorUtils.handleError(UnauthorizedException(jsonObject.getString("detail")), "")
                else if (jsonObject.has("non_field_errors")) {
                    errorUtils.handleError(UnauthorizedException(jsonObject.getString("non_field_errors")), "")
                } else {
            ...
            return Resource.error(
                errorUtils.handleError(ValidationException(R.string.something_went_wrong.asString()), "")
            )
        } else {
